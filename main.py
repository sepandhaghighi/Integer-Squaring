...
def IntegerSquaringModified(Input_integer,Base=10):
    '''
     Integer Squaring Algorithm Modified
     Book : Cryptographic Engineering (Serdar S¨ uer Erdem, Tuˇ grul Yanık, and C ¸ etin Kaya Koc)
     Chapter : 5
     Page : 80
    :param Input_integer : Input Integer as string with space separation between digit "0 11 12" in base 13
    ...
    try:
        ...
        for number in IntegerList:
            if number>Base-1:
                raise Exception("[Error] Wrong Number In This Base")
        IntegerList.reverse()
        IntegerLength=len(IntegerList)
        I=[]
        d=[]
...
def IntegerMultiplication(Input_integer_1,Input_integer_2,Base=10):
    ...
    try:
        ...
        for number in IntegerList1:
            if number>Base-1:
                raise Exception("[Error] Wrong Number_1 In This Base")
        for number in IntegerList2:
            if number>Base-1:
                raise Exception("[Error] Wrong Number_2 In This Base")
